...
// tomatoes[k][i][j] = cur;
// if (tomatoes[k][i][j] == 1) q.add(new PointXYZ(k, i, j)); // 익은 토마토의 층행열 정보를
// 큐에 추가
// }
// }
// }
...
// if (curZ>=0 && curX>=0 && curY>=0 && curZ<h && curX<n && curY<m) { // 토마토 상자
// 경계 내에 있는 토마토들만 유효
// // System.out.println("tomatoes[curZ][curX][curY]:
// "+tomatoes[curZ][curX][curY]);
...
// if (tomatoes[k][i][j] ==0) return -1; // 익은 정도를 검사하였는데도 0이 있다면 익지 못 하는 토마토가
// 있는 것이므로 -1 리턴
